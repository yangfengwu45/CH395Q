...
//缓存服务器发送的响应头部的数据
unsigned char http_response_data[1024];
...
<html>\
    ...
    \
    <body>\
        <form  action=\"LEDCONTROL.CGI\">\
        LED\
				&nbsp  <input value=\"1\" name=\"led1\" type=\"radio\">打开\
				&nbsp 	<input value=\"2\" name=\"led1\" type=\"radio\">关闭\
        <br>\
        <br>\
...
{	
	...
	if(sock_int_socket & SINT_STAT_RECV)                              
	{
		...
		ParseHttpRequest(&http_request,recv_buff);
		...
		switch (http_request.METHOD)  
		{
			...
			case METHOD_GET:
				...
				{
					printf("\r\nPTYPE_HTML--------------------------\r\n");
					MakeHttpResponse(http_response_data,http_request.TYPE,sizeof(login_data));
					//发送响应头部
					CH395SendData(sockindex,(UINT8*)http_response_data,strlen((char*)http_response_data));
					delay_ms(10);
					/*发送响应正文*/
					CH395SendData(sockindex,(unsigned char*)login_data,sizeof(login_data));
				}
				else if(http_request.TYPE == PTYPE_CGI)
				{
					printf("\r\nPTYPE_CGI--------------------------\r\n");
					MakeHttpResponse(http_response_data,http_request.TYPE,sizeof(control_data));
					//发送响应头部
					CH395SendData(sockindex,(UINT8*)http_response_data,strlen((char*)http_response_data));
					delay_ms(10);
					/*发送响应正文*/
					CH395SendData(sockindex,(unsigned char*)control_data,sizeof(control_data));
				}
			break;
		}
	}
