...
* Version            : V1.1
* Date               : 2014/8/1
* Description        : CH395芯片FTP服务器应用-数据处理部分
...
void  mInitFtpList( )
{
  ...
  sprintf(ListRoot,"%s%4d A.txt\r\n%s%4d B.txt\r\n%sUSER\r\n",FileNature,strlen(RootBuf1),FileNature,strlen(RootBuf1),ListNature); // 当前目录
  ...
  ftp.BufStatus = ftp.BufStatus|0x03;                                           /* buf1和buf2不为空*/
} 
...
void CH395_FTPListRenew( UINT8 index )
{
  ...
  printf("NameFile:\n%s\n",NameFile);
    ...
    else if(index == 0||index == 5){                                            /* 替换文件后者重命名文件*/
      i = 0;
      while((strncmp(pFileName,&ListRoot[i], strlen(pFileName)))&&i<strlen(ListRoot)) i++;
...
void CH395_FTPFileRead( char *pName )
{
  ...
  memset((void *)SendBuf,'\0',sizeof(SendBuf));
    ...
    len = CH395_FTPFileSize( pName );                                            /* 获取要读取文件长度*/
    if(len < SendBufLen){
      memset(SendBuf,'\0',sizeof(SendBuf));
