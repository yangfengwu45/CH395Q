...
void CH395ServerSocketInitOpen(UINT8 index)
{
    ...
    CH395SetSocketProtType(index,SockInf[index].ProtoType);          /* 设置socket 协议类型 */
    CH395SetSocketSourPort(index,SockInf[index].SourPort);           /* 设置socket 源端口 */
...
void CH395ClientSocketInitOpen(UINT8 index)
{
    ...
    CH395SetSocketDesIP(index,SockInf[index].IPAddr);                  /* 设置socket 目标IP地址 */         
    CH395SetSocketProtType(index,SockInf[index].ProtoType);            /* 设置socket 协议类型 */
    CH395SetSocketDesPort(index,SockInf[index].DesPort);               /* 设置socket 目的端口 */
    CH395SetSocketSourPort(index,SockInf[index].SourPort);             /* 设置socket 源端口 */
    i = CH395OpenSocket(index);                                        /* 打开socket  */
    mStopIfError(i);                                                   /* 检查是否成功 */
...
{
    memset(&SockInf[socketid],0,sizeof(SockInf[socketid]));             /* 将对应SockInf全部清零*/
    SockInf[socketid].SourPort = 21;                                    /* 源端口 */
    SockInf[socketid].ProtoType = PROTO_TYPE_TCP;                       /* TCP模式 */
    SockInf[socketid].TcpMode = TCP_SERVER_MODE;                        /* TCP服务器模式 */
...
void CH395_FTPDatServer( UINT8 socketid,UINT16 *port )
{
    ...
    SerPort++;
    memset(&SockInf[socketid],0,sizeof(SockInf[socketid]));             /* 将对应SockInf全部清零*/
    SockInf[socketid].SourPort = 256*5 + SerPort;                       /* 源端口 */
...
{
    memset(&SockInf[socketid],0,sizeof(SockInf[socketid]));             /* 将对应SockInf全部清零*/
    memcpy(SockInf[socketid].IPAddr,pAddr,4);                           /* 将目的IP地址写入 */
    SockInf[socketid].DesPort = port;                                   /* 目的端口 */
    SockInf[socketid].SourPort = 20;                                    /* 源端口 */
    SockInf[socketid].ProtoType = PROTO_TYPE_TCP;                       /* TCP模式 */
    SockInf[socketid].TcpMode = TCP_CLIENT_MODE;                        /* TCP客户端模式 */
...
void CH395_FTPGetSockeID( UINT8 socketid )
{
     ...
     printf("ftp.SocketCtl = %d\n",(UINT16)ftp.SocketCtl);
               ...
               i = CH395CloseSocket(socketid);                               /* 只接收一个客户端连接      */
               mStopIfError( i ); 
#if DEBUG
